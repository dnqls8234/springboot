...
(function ($)
{
    ...
    $.extend($.jqx._jqxPasswordInput.prototype, {
        ...
        {
            var that = this,
                currentPassword = that.element.value,
                hasPlaceholder = 'placeholder' in that.element;
            ...
            if (key == 'placeHolder')
            {
                if (this._browserCheck)
                {
                    ...
                    } else
                    {
                        ...
                        {
                            object.element.setAttribute('type', 'text');
                            object.element.value = value;
                        } else if (object.element.value == oldvalue)
                        {
                            object.element.value = value;
                        }
                    }
        ...
        resize: function (width, height)
        {
            this.width = width;
            this.height = height;
            this.element.style.width = this._toPx(width);
            this.element.style.height = this._toPx(height);
        },
        ...
        _setAttributes: function ()
        {
            ...
            widget.style.width = that._toPx(that.width);
            widget.style.height = that._toPx(that.height);
            ...
            that.addHandler(widget, 'blur.passwordinput' + that.widgetID, function ()
            {
                ...
                if (that.placeHolder && that._browserCheck && !('placeholder' in that.element) && widget.value === '')
                {
                    that.element.value = that.placeHolder;
                    that.element.setAttribute('type', 'text');
                }
        ...
        {
            var that = this,
                classToAdd = 'jqx-widget jqx-widget-content jqx-input jqx-rc-all';
        ...
        _showStrength: function ()
        {
            ...
            if (that.showStrength)
            {
                if (that.host.jqxTooltip)
                {
                    ...
                    {
                        // custom content
                        var password = that.element.value;
                        if (!('placeholder' in that.element) && that._browserCheck && password == that.placeHolder)
                        {
                            password = '';
                        }
