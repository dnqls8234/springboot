...
(function ($) {
    ...
    $.extend($.jqx._jqxPathControl.prototype, {
            ...
            if (value !== oldvalue) {
                switch (key) {
                    case 'width':
                        ...
                        object._input.css('width', object.host.outerWidth() - object._button.outerWidth() - 7);
                        break;
                    case 'height':
                    ...
                    case 'rtl':
                        if (value === true) {
                            ...
                            object._button.removeClass(that.toThemeProperty('jqx-path-control-button-ltr'));
                            object._button.addClass(that.toThemeProperty('jqx-path-control-button-rtl'));
                        } else {
                            object._input.removeClass(object.toThemeProperty('jqx-path-control-input-rtl'));
                            object._button.removeClass(that.toThemeProperty('jqx-path-control-button-rtl'));
                            object._button.addClass(that.toThemeProperty('jqx-path-control-button-ltr'));
                        }
        ...
        _constructPath: function (initialPath) {
            ...
            if (initialPath.indexOf('\\') !== -1) {
                var tempPath = initialPath;
                var colonIndex = initialPath.indexOf(':');
                if (colonIndex !== -1) {
                    that._drive = initialPath.charAt(0);
                    tempPath = initialPath.slice(colonIndex + 2);
                }
                that._pathArray = tempPath.split('\\');
            } else if (initialPath.indexOf('/') !== -1) {
                that._pathArray = initialPath.split('/');
            } else if (initialPath.indexOf(that.customDirectorySeparator) !== -1) {
                that._pathArray = initialPath.split(that.customDirectorySeparator);
            } else {
                that._pathArray.push(initialPath);
            }
        ...
        _appendElements: function () {
            ...
            that._input.css('width', that.host.outerWidth() - that._button.outerWidth() - 7);
            ...
            $.jqx.utilities.resize(that.host, function () {
                if (typeof that.width === 'string' && that.width.charAt(that.width.length - 1) === '%') {
                    that._input.css('width', that.host.outerWidth() - that._button.outerWidth() - 7);
                }
            });
        },
        ...
        _addHandlers: function () {
            ...
            that.addHandler(that._input, 'keydown.jqxPathControl' + id, function (event) {
                ...
                if (that.pathFormat === 'windows') {
                    if (restricted || keyCode === 191 /* forward slash */) {
                        event.preventDefault();
                    } else if ((keyCode === 186 || keyCode === 59 && $.jqx.browser.mozilla) && event.shiftKey === true /* colon */) {
                        if (that._input.val().indexOf(':') !== -1) { // only one colon is allowed
                            event.preventDefault();
                        }
                    }
                } else if (that.pathFormat === 'unix') {
                    if (restricted || keyCode === 220 /* backslash */ || ((keyCode === 186 || keyCode === 59 && $.jqx.browser.mozilla) && event.shiftKey === true) /* colon */) {
                        event.preventDefault();
                    }
                }
            });
