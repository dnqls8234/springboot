...
(function ($) {
    ...
    $.extend($.jqx._jqxDataTable.prototype, {
        ...
        _render: function () {
            ...
            if (that.filterable) {
                that.filter[0].style.visibility = "inherit";
            }
            else {
                that.filter[0].style.visibility = "hidden";
            }
            ...
            if (that.columns == null) {
                that.columns = new $.jqx.dataCollection(that.element);
            }
            else {
            ...
            that._lastHostWidth = that.host.width();
            that._lastHostHeight = that.host.height();
            ...
            that.gridcontent = that.gridcontent || $('<div style="width: 100%; overflow: hidden; position: absolute;"></div>');
            that.gridcontent.remove();
            ...
            that.content.append(that.gridcontent);

            that._rendercolumnheaders();

            that.tableheight = null;

            that.gridcontent.find('#contenttable' + that.element.id).remove();
            if (that.table != null) {
                that.table.remove();
                that.table = null;
            }
            ...
            that.gridcontent.addClass(that.toTP('jqx-grid-content'));
            that.gridcontent.addClass(that.toTP('jqx-widget-content'));
            that.gridcontent.append(that.table);
            that._renderrows();
            ...
            if (that.renderStatusBar) {
                that.renderStatusBar(that.statusbar);
            }
            if (that.rendertoolbar) {
                that.rendertoolbar(that.toolbar);
            }
            if (that.showAggregates) {
                that._updateaggregates();
            }
                ...
                var applychanges = function (that, rowid) {
                    ...
                    if (!$.isArray(rowid)) {
                        ...
                        if (that.rowsByKey[rowid]) {
                            success = true;
                            if (that.rowinfo[rowid] && that.rowinfo[rowid].selected) {
